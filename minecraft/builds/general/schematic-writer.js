const _teWriter = new TextEncoder();
...
function createNBTWriter() {
  ...
  function writeString(str) {
    const encoded = _teWriter.encode(str);
...
const CHUNK_BUF_SIZE = CHUNK_SIZE + 10;
...
class SchematicStreamWriter {
  constructor(width, height, length) {
    ...
    this.currentChunk = new Uint8Array(CHUNK_BUF_SIZE);
    this.chunkOffset = 0;
  ...
  addBlock(blockName) {
    ...
    while ((value & 0xFFFFFF80) !== 0) {
      this.currentChunk[this.chunkOffset++] = (value & 0x7F) | 0x80;
      value >>>= 7;
    }
    this.currentChunk[this.chunkOffset++] = value & 0x7F;
    
    if (this.chunkOffset >= CHUNK_SIZE) {
      this.blockDataChunks.push(this.currentChunk.subarray(0, this.chunkOffset));
      this.currentChunk = new Uint8Array(CHUNK_BUF_SIZE);
      this.chunkOffset = 0;
    }
