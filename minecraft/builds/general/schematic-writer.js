...
class SchematicStreamWriter {
  ...
  _flushChunk() {
    this.blockDataChunks.push(this.currentChunk.subarray(0, this.currentChunkOffset));
    this.currentChunk = new Uint8Array(this.chunkSize);
    this.currentChunkOffset = 0;
  }

  _writeByte(b) {
    this.currentChunk[this.currentChunkOffset++] = b;
    if (this.currentChunkOffset === this.chunkSize) {
      this._flushChunk();
    }
  }
