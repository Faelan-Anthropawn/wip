const _tdMca = new TextDecoder('utf-8');
...
function parseNBT(buffer) {
  ...
  function readString() {
    ...
    const str = _tdMca.decode(buffer.subarray(offset, offset + length));
    offset += length;
    return str;
  }
  ...
  function readByteArray() {
    ...
    const arr = Array.from(buffer.subarray(offset, offset + length));
    offset += length;
    return arr;
  }
...
function getBlockFromChunk(chunkData, localX, y, localZ) {
  ...
  if (section.block_states && section.block_states.palette) {
    ...
    if (!section.block_states._loArr) {
      ...
      for (let j = 0; j < n; j++) {
        ...
        loArr[j] = Number(v & 0xFFFFFFFFn) >>> 0;
        hiArr[j] = Number(v >> 32n) >>> 0;
      }
      section.block_states._loArr = loArr;
      section.block_states._hiArr = hiArr;
    }

    const loArr = section.block_states._loArr;
    const hiArr = section.block_states._hiArr;
    const mask = (bitsPerBlock < 32) ? (1 << bitsPerBlock) - 1 : 0xFFFFFFFF;
    const startBit = bitOffset;
    const endBit = bitOffset + bitsPerBlock;

    let paletteIndex;
    if (endBit <= 32) {
      paletteIndex = (loArr[longIndex] >>> startBit) & mask;
    } else if (startBit >= 32) {
      paletteIndex = (hiArr[longIndex] >>> (startBit - 32)) & mask;
    } else {
      const shift = 32 - startBit;
      paletteIndex = ((loArr[longIndex] >>> startBit) | (shift < 32 ? hiArr[longIndex] << shift : 0)) & mask;
    }
