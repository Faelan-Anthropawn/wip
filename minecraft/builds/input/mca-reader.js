...
function parseNBT(buffer) {
  ...
  function readString() {
    ...
    const str = sharedTextDecoder.decode(buffer.subarray(offset, offset + length));
    offset += length;
    return str;
  }
  ...
  function readLongArray() {
    const length = readInt();
    const arr = new Int32Array(length * 2);
    for (let i = 0; i < length; i++) {
      arr[i * 2]     = readInt();
      arr[i * 2 + 1] = readInt();
    }
...
function getBlockFromChunk(chunkData, localX, y, localZ) {
  ...
  if (section.block_states && section.block_states.palette) {
    ...
    const pairBase = longIndex * 2;
    if (pairBase + 1 >= blockData.length) return palette[0].Name || null;

    const hi = blockData[pairBase] >>> 0;
    const lo = blockData[pairBase + 1] >>> 0;
    const mask = bitsPerBlock < 32 ? (1 << bitsPerBlock) - 1 : 0xFFFFFFFF;

    let paletteIndex;
    if (bitOffset + bitsPerBlock <= 32) {
      paletteIndex = (lo >>> bitOffset) & mask;
    } else if (bitOffset >= 32) {
      paletteIndex = (hi >>> (bitOffset - 32)) & mask;
    } else {
      paletteIndex = (((lo >>> bitOffset) | (hi << (32 - bitOffset))) >>> 0) & mask;
    }
