...
import { createNbtBuffer, buildStructureFromBlocks } from '../outputs/structure-converter.js';
...
async function convertWorld() {
  ...
  try {
    ...
    if (outputFormat === 'command_dump') {
      ...
      worldShowStatus(`Building .mcstructure (${currentSchem.width}×${currentSchem.height}×${currentSchem.length})...`, 'info');
      convertButton.textContent = 'Building .mcstructure...';
      logToConsole(`Building .mcstructure (${currentSchem.width}×${currentSchem.height}×${currentSchem.length})...`, 'info');
      const structureData = buildStructureFromBlocks(currentSchem, getKeyAt);

      if (!structureData) {
        worldShowStatus('Region is empty — no blocks to export', 'error');
        logToConsole('Error: Region is empty — no blocks to export', 'error');
        convertButton.disabled = false;
        convertButton.textContent = 'Begin Conversion';
        return;
      }

      logToConsole('Encoding to NBT format...', 'info');
      ...
      worldShowStatus(`✅ Success! Created .mcstructure`, 'success');
