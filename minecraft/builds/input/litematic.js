...
const _teLitematic = new TextEncoder();

function longsToHiLo(longArr) {
...
function extractPacked32(lo, hi, numLongs, bitPos, bitsPerEntry) {
  const longIdx = (bitPos / 64) | 0;
  if (longIdx >= numLongs) return 0;
  const startBit = bitPos - longIdx * 64;
  const endBit = startBit + bitsPerEntry;
  const mask = (bitsPerEntry < 32) ? (1 << bitsPerEntry) - 1 : 0xFFFFFFFF;

  let val;
  if (endBit <= 32) {
    val = (lo[longIdx] >>> startBit) & mask;
  } else if (startBit >= 32) {
    val = (hi[longIdx] >>> (startBit - 32)) & mask;
  } else if (endBit <= 64) {
    const shift = 32 - startBit;
    val = ((lo[longIdx] >>> startBit) | (shift < 32 ? hi[longIdx] << shift : 0)) & mask;
  } else {
    const bitsFromFirst = 64 - startBit;
    let firstPart;
    if (startBit >= 32) {
      firstPart = hi[longIdx] >>> (startBit - 32);
    } else {
      const shift = 32 - startBit;
      firstPart = (lo[longIdx] >>> startBit) | (shift < 32 ? hi[longIdx] << shift : 0);
    }
    firstPart &= (1 << bitsFromFirst) - 1;
    if (longIdx + 1 >= numLongs) return firstPart & mask;
    const bitsFromNext = bitsPerEntry - bitsFromFirst;
    const secondPart = lo[longIdx + 1] & ((1 << bitsFromNext) - 1);
    val = firstPart | (secondPart << bitsFromFirst);
  }
  return val >>> 0;
}
...
async function streamMultipleRegions(regionEntries, metadata, blockStream) {
  ...
  for (const [, region] of regionEntries) {
    ...
    const { lo, hi } = longsToHiLo(longArr);
    const numLongs = longArr.length;
    ...
    for (let i = 0; i < vol; i++) {
      ...
      const paletteIdx = extractPacked32(lo, hi, numLongs, i * bitsPerBlock, bitsPerBlock);
      ...
      if (gx >= 0 && gx < width && gy >= 0 && gy < height && gz >= 0 && gz < length) {
        const key = gx + gz * width + gy * width * length;
...
async function streamPackedBlockStates(longArr, count, bitsPerEntry, palette, width, length, offsetX, offsetY, offsetZ, blockStream) {
  ...
  for (let i = 0; i < count; i++) {
    const paletteIdx = extractPacked32(lo, hi, numLongs, i * bitsPerEntry, bitsPerEntry);
...
function encodeNBT(root) {
  ...
  function writeTag(type, name, value) {
    ...
    if (name != null) {
      const nb = _teLitematic.encode(name);
    ...
    switch (type) {
      ...
      case 8: {
        const sb = _teLitematic.encode(value);
