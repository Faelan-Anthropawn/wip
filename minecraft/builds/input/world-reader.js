...
async function extractRegionStreaming(mcaFiles, x1, y1, z1, x2, y2, z2, onBlock, onProgress) {
  ...
  for (const regionKey of neededRegions) {
    ...
    for (const [chunkKey, chunkData] of chunks) {
      ...
      parsedRegions.set(absoluteChunkX * CHUNK_KEY_MUL + absoluteChunkZ, chunkData);
    }
  }
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let z = minZ; z <= maxZ; z++) {
      ...
      let lastChunkKey = -999999999999;
      let lastChunk = null;

      for (let x = minX; x <= maxX; x++) {
        const chunkX = (x >> 4);
        const chunkKey = chunkX * CHUNK_KEY_MUL + chunkZ;
        if (chunkKey !== lastChunkKey) {
          lastChunk = parsedRegions.get(chunkKey);
        ...
        if (lastChunk) {
          ...
          const foundBlock = getBlockFromChunk(lastChunk, localX, y, localZ);
          if (foundBlock) {
  ...
  setTimeout(async () => {
    try {
        ...
        for (let z = minZ; z <= maxZ; z++) {
          ...
          for (let x = minX; x <= maxX; x++) {
            ...
            if (chunkKey !== lastChunkKey) {
              lastChunk = parsedRegions.get(chunkKey);
              lastChunkKey = chunkKey;
              if (!lastChunk && !missingChunks.has(chunkKey)) {
      ...
      if (missingChunks.size > 0) {
        console.warn(`[WorldReader] Missing ${missingChunks.size} chunks`);
      }
