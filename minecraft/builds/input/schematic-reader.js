...
const _tdSchem = new TextDecoder("utf-8");
...
function decodePackedBlockStates(longArr, count, bitsPerEntry) {
  ...
  for (let i = 0; i < numLongs; i++) {
    const v = longArr[i];
    const unsigned = v < 0n ? v + 0x10000000000000000n : v;
    lo[i] = Number(unsigned & 0xFFFFFFFFn) >>> 0;
    hi[i] = Number(unsigned >> 32n) >>> 0;
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (endBit <= 32) {
      ...
      const secondPart = longIdx + 1 < numLongs ? lo[longIdx + 1] & ((1 << bitsFromNext) - 1) : 0;
