...
const _teStructure = new TextEncoder();
...
function writeStringPayload(buffer, offset, text) {
  ...
  const utf8Bytes = _teStructure.encode(text);
...
function makeBlockKey(fullName, stateEntries) {
  if (stateEntries.length === 0) return fullName;
  let key = fullName + '[';
  for (let i = 0; i < stateEntries.length; i++) {
    if (i > 0) key += ',';
    key += stateEntries[i][0] + '=' + stateEntries[i][1];
  }
  return key + ']';
}
    ...
    for (let y = minY; y < minY + height; y++) {
      for (let z = minZ; z < minZ + depth; z++) {
        ...
        if (blockData) {
          ...
          const blockKey = makeBlockKey(fullName, stateEntries);
...
function buildStructureFromBlocks(schem, getKeyAt) {
  const { width: w, height: h, length: l } = schem;

  const belowBarriers = getKeyAt.belowBoundsBarriers || [];
  const hasBarriers = belowBarriers.length > 0;
  const baseY = hasBarriers ? 1 : 0;
  const structHeight = h + baseY;

  const totalSize = w * structHeight * l;
  const blockIndicesLayer0 = new Array(totalSize).fill(-1);

  const structIdx = (sx, sy, sz) => sx + sz * w + sy * w * l;

  const uniqueBlocks = new Map();
  const palette = [];

  const getOrAddPaletteIndex = (blockName, states) => {
    const fullName = blockName.includes(':') ? blockName : `minecraft:${blockName}`;
    const stateEntries = Object.entries(states || {}).sort((a, b) => a[0].localeCompare(b[0]));
    const blockKey = makeBlockKey(fullName, stateEntries);
    if (!uniqueBlocks.has(blockKey)) {
      const idx = palette.length;
      uniqueBlocks.set(blockKey, idx);
      palette.push({ name: fullName, states: states || {}, version: 18163713 });
      return idx;
    }
    return uniqueBlocks.get(blockKey);
  };

  let blockCount = 0;

  if (hasBarriers) {
    const barrierIdx = getOrAddPaletteIndex('minecraft:barrier', {});
    const placed = new Set();
    for (const b of belowBarriers) {
      const key = b.x * 65536 + b.z;
      if (!placed.has(key) && b.x >= 0 && b.x < w && b.z >= 0 && b.z < l) {
        placed.add(key);
        blockIndicesLayer0[structIdx(b.x, 0, b.z)] = barrierIdx;
        blockCount++;
      }
    }
  }
  ...
  for (let x = 0; x < w; x++) {
    for (let y = 0; y < h; y++) {
      for (let z = 0; z < l; z++) {
        const schemFlatIdx = x + z * w + y * w * l;
        const key = getKeyAt(schemFlatIdx);
        if (!key) continue;
        const [blockName, states] = parseCmdStructBlockWithStates(key);
        blockIndicesLayer0[structIdx(x, y + baseY, z)] = getOrAddPaletteIndex(blockName, states);
        blockCount++;
      }
    }
  }
  ...
  const blockIndicesLayer1 = new Array(totalSize).fill(-1);

  return {
    format_version: 1,
    size: [w, structHeight, l],
    structure: {
      block_indices: [blockIndicesLayer0, blockIndicesLayer1],
      entities: [],
      palette: {
...
function buildChunkStructureFromBlocks(schem, getKeyAt, chunkBounds, options = {}) {
  const { width: w, height: h, length: l } = schem;
  ...
  const hasBarriers = chunkBarriers.length > 0;
  const baseY = hasBarriers ? 1 : 0;
  const structHeight = chunkSize + baseY;

  const totalSize = chunkSize * structHeight * chunkSize;
  const blockIndicesLayer0 = new Array(totalSize).fill(-1);

  const structIdx = (sx, sy, sz) => sx + sz * chunkSize + sy * chunkSize * chunkSize;
  ...
  if (hasBarriers) {
    const barrierIdx = getOrAddPaletteIndex('minecraft:barrier', {});
    const placed = new Set();
    for (const b of chunkBarriers) {
      const key = b.x * 65536 + b.z;
      if (!placed.has(key)) {
        placed.add(key);
        blockIndicesLayer0[structIdx(b.x - minX, 0, b.z - minZ)] = barrierIdx;
        hasAnyBlock = true;
      }
    }
  }
  ...
  for (let x = minX; x <= maxX; x++) {
    for (let y = minY; y <= maxY; y++) {
      for (let z = minZ; z <= maxZ; z++) {
        ...
        if (!key) continue;
        const [blockName, states] = parseCmdStructBlockWithStates(key);
        blockIndicesLayer0[structIdx(x - minX, (y - minY) + baseY, z - minZ)] = getOrAddPaletteIndex(blockName, states);
        hasAnyBlock = true;
      }
    }
  }
  ...
  return {
    format_version: 1,
    size: [chunkSize, structHeight, chunkSize],
    structure: {
      block_indices: [blockIndicesLayer0, blockIndicesLayer1],
      entities: [],
      palette: {
...
export {
  ...
  padStructureWithAir,
  buildStructureFromBlocks,
  buildChunkStructureFromBlocks,
};
