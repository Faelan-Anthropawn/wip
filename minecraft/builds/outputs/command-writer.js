...
function generateCommands(schem, getKeyAt, options = {}) {
  ...
  if (getKeyAt.belowBoundsBarriers && getKeyAt.belowBoundsBarriers.length > 0) {
    ...
    for (const b of barriers) barrierSet.add(`${b.x},${b.y},${b.z}`);
    ...
    for (const barrier of barriers) {
      ...
      while (barrierSet.has(`${x1 + 1},${y0},${z0}`) && !barrierVisited.has(`${x1 + 1},${y0},${z0}`)) {
        x1++;
      }
...
function generateChunkCommands(schem, getKeyAt, chunkBounds, options = {}) {
  ...
  if (getKeyAt.belowBoundsBarriers && getKeyAt.belowBoundsBarriers.length > 0) {
    ...
    const barrierSet = new Set();
    for (const b of chunkBarriers) barrierSet.add(`${b.x},${b.y},${b.z}`);
    ...
    for (const barrier of chunkBarriers) {
      ...
      while (x1 + 1 <= maxX && barrierSet.has(`${x1 + 1},${y0},${z0}`) && !barrierVisited.has(`${x1 + 1},${y0},${z0}`)) {
        x1++;
      }
