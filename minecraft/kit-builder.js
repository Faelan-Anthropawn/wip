...
function createEnchantRow(ench, itemIdx, enchIdx) {
  ...
  row.innerHTML = `
    ...
    <div class="flex items-center gap-2">
      <label for="kit-ench-lvl-${itemIdx}-${enchIdx}" class="text-xs text-slate-400 shrink-0">Level</label>
      <input type="number" min="1" max="32767" value="${ench.level}"
        id="kit-ench-lvl-${itemIdx}-${enchIdx}" name="kit-ench-lvl-${itemIdx}-${enchIdx}"
  ...
  card.innerHTML = `
    <div class="flex gap-3 items-center">
      ...
      <input type="text" placeholder="e.g. minecraft:diamond"
        id="kit-item-id-${index}" name="kit-item-id-${index}"
        aria-label="Item ${index + 1} identifier"
    ...
    <div class="kit-options-bar flex flex-col gap-3 p-3 rounded-lg bg-surface-900/50 border border-slate-700/30" style="display:${optionsDisplay}">
      ...
      <div class="grid grid-cols-2 gap-3">
        <label class="flex flex-col gap-1">
          ...
          <input type="number" min="1" max="99" value="${item.count}"
            id="kit-item-count-${index}" name="kit-item-count-${index}"
        ...
        <label class="flex flex-col gap-1">
          ...
          <input type="number" min="0" max="32767" value="${item.damage}"
            id="kit-item-damage-${index}" name="kit-item-damage-${index}"
      ...
      <div class="flex flex-col gap-1 kit-name-section">
        <label for="kit-item-name-${index}" class="text-xs text-slate-400 font-medium">Display Name</label>
        <input type="text" placeholder="Custom name (supports §colour codes)"
          id="kit-item-name-${index}" name="kit-item-name-${index}"
