...
import { createNbtBuffer, buildStructureFromBlocks } from './minecraft/builds/outputs/structure-converter.js';
...
convertButton.addEventListener('click', async () => {
  ...
  try {
    ...
    if (outputFormat === 'commands') {
      ...
      appShowStatus(`Building .mcstructure (${currentSchem.width}x${currentSchem.height}x${currentSchem.length})...`, 'info');
      logToConsole(`Building .mcstructure (${currentSchem.width}x${currentSchem.height}x${currentSchem.length})...`, 'info');
      const structureData = buildStructureFromBlocks(currentSchem, getKeyAt);

      if (!structureData) {
        appShowStatus('Structure is empty — no blocks to export', 'error');
        logToConsole('Error: Structure is empty — no blocks to export', 'error');
        convertButton.disabled = false;
        return;
      }
      ...
      appShowStatus(`✅ Success! Created .mcstructure`, 'success');
